...
if torch.cuda.is_available():
    print(f"\n使用 GPU 运行: {torch.cuda.get_device_name(0)}")
    print(
        f"GPU 内存总量: {torch.cuda.get_device_properties(0).total_memory / 1024 / 1024 / 1024:.2f} GB"
    )
    print(f"CUDA 版本: {torch.version.cuda}\n")
else:
    print("\n注意: 使用 CPU 运行，这可能会导致测试速度较慢\n")
    ...
    with torch.no_grad():
        ...
        ssim = (
            ...
            * kornia.losses.ssim_loss(
                encoded_images.detach(), images, window_size=5
            ).item()
        )
